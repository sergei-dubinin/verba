...
- `src/server/` — сервисы поверх Prisma; `db.ts` — клиент базы, `auth/` — пользователи, пароли, сессии; `recordings/` — список, автоназвание, результат обработки; `transcript/` — транскрипт записи; `stt/` — формат ответа провайдера; `time.ts` — часовой пояс приложения.
